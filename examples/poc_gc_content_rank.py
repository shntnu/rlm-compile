#!/usr/bin/env -S uv run --script
# /// script
# requires-python = ">=3.11,<3.14"
# dependencies = [
#     "rlms",
#     "python-dotenv",
# ]
# ///
"""Tiny live RLM POC: rank human DNA sequences by GC content.

The task is pure char-counting on one CSV. The root LM should close it in a
single REPL block without ever calling `rlm_query` - useful when you want a
fresh trace whose recovered program is deterministic Python with zero
recorded sub-LLM calls.

Run without spending tokens:
    uv run examples/poc_gc_content_rank.py --gold-only

Run a real RLM trace:
    uv run examples/poc_gc_content_rank.py

Then compile the generated trace:
    uv run compile.py traces/<new_trace>.jsonl.gz compiled/gc_content_rank.py
"""
from __future__ import annotations

import argparse
import csv
import io
import json
import os
from pathlib import Path
from typing import Any

from dotenv import load_dotenv
from rlm import RLM
from rlm.logger import RLMLogger


ROWS = [
    {"sequence_id": "HS_001", "organism": "human", "sequence": "ATGCGCGCGGCGCATCGCGGCG"},
    {"sequence_id": "HS_002", "organism": "human", "sequence": "ATATATATATATATATATATAT"},
    {"sequence_id": "HS_003", "organism": "human", "sequence": "GCGCGCGCATGCGCATGCGCAT"},
    {"sequence_id": "HS_004", "organism": "mouse", "sequence": "GGCGCGCGCGCGCGCGCGCGCG"},
    {"sequence_id": "HS_005", "organism": "human", "sequence": "ATGCATGCATGCATGCATGCAT"},
    {"sequence_id": "HS_006", "organism": "human", "sequence": "GCGCATATATATATATATATAT"},
]


def build_context() -> str:
    buffer = io.StringIO()
    writer = csv.DictWriter(
        buffer, fieldnames=["sequence_id", "organism", "sequence"]
    )
    writer.writeheader()
    writer.writerows(ROWS)
    return buffer.getvalue()


def gc_content(seq: str) -> float:
    upper = seq.upper()
    gc = sum(1 for ch in upper if ch in ("G", "C"))
    return round(gc / len(upper), 4)


def compute_gold(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    hits = [
        {
            "sequence_id": str(row["sequence_id"]),
            "gc_content": gc_content(str(row["sequence"])),
        }
        for row in rows
        if row["organism"] == "human"
    ]
    return sorted(hits, key=lambda r: (-r["gc_content"], r["sequence_id"]))[:3]


def root_prompt() -> str:
    return """The variable `context` contains one CSV table with columns:
sequence_id,organism,sequence.

Return the top 3 human sequences by GC content.

Rules:
- Keep only rows where organism == human.
- gc_content = (count of G + count of C) / length of sequence, on the
  uppercased sequence string.
- Round gc_content to 4 decimal places.
- Sort by descending gc_content, then sequence_id ascending.

Return ONLY compact JSON, no markdown. The JSON must be an array of objects
with keys: sequence_id, gc_content."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--backend", default="openrouter")
    parser.add_argument("--model", default="anthropic/claude-sonnet-4.5")
    parser.add_argument("--max-iterations", type=int, default=6)
    parser.add_argument("--max-depth", type=int, default=1)
    parser.add_argument("--log-dir", type=Path, default=Path("./traces"))
    parser.add_argument(
        "--gold-only",
        action="store_true",
        help="Print gold answer without calling an API.",
    )
    parser.add_argument("--context-out", type=Path, default=None)
    parser.add_argument("--gold-out", type=Path, default=None)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    load_dotenv()
    context = build_context()
    gold = compute_gold(ROWS)

    print("Context:")
    print(context, end="")
    print()
    print(f"Gold answer: {json.dumps(gold, indent=2)}")
    print()

    if args.context_out is not None:
        args.context_out.write_text(context)
        print(f"Wrote context packet: {args.context_out}")
    if args.gold_out is not None:
        args.gold_out.write_text(json.dumps(gold, indent=2) + "\n")
        print(f"Wrote gold answer: {args.gold_out}")
    if args.context_out is not None or args.gold_out is not None:
        print()

    if args.gold_only:
        return 0

    api_key_name = (
        "OPENROUTER_API_KEY" if args.backend == "openrouter" else "OPENAI_API_KEY"
    )
    api_key = os.getenv(api_key_name)
    if not api_key:
        raise SystemExit(f"{api_key_name} is required for backend={args.backend!r}")

    rlm = RLM(
        backend=args.backend,
        backend_kwargs={"api_key": api_key, "model_name": args.model},
        environment="local",
        max_depth=args.max_depth,
        max_iterations=args.max_iterations,
        logger=RLMLogger(log_dir=str(args.log_dir)),
        verbose=True,
    )

    result = rlm.completion(prompt=context, root_prompt=root_prompt())

    print()
    print("Model answer:")
    print(result.response)
    print()
    print("Gold answer:")
    print(json.dumps(gold, indent=2))
    try:
        model_answer = json.loads(result.response)
        print()
        print(f"Matches gold: {model_answer == gold}")
    except json.JSONDecodeError as exc:
        print()
        print(f"Could not parse model answer as JSON: {exc}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
