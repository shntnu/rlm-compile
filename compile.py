...
Generated artifact LLM modes:
  ...
  live    Make ordinary OpenAI-compatible chat-completion calls.
          Uses OPENROUTER_API_KEY with OpenRouter defaults, or OPENAI_API_KEY
          with OpenAI defaults. Honors the matching BASE_URL and MODEL env vars.
...
def _build_readable_preamble(*, log_name: str, replay_name: str, recovered_name: str, module_name: str) -> str:
    """Build the preamble for a runnable recovered script."""
    # Built programmatically to avoid triple-quote delimiter conflicts
    docstring = (
        f"Recovered program from {log_name}.\n"
        "\n"
        "Runnable standalone script compiled from an RLM trace. Provides the same\n"
        f"algorithm as {replay_name} in a flat, readable form.\n"
        "\n"
        "CLI:\n"
        f"    python {recovered_name} --context input.txt\n"
        f"    python {recovered_name} --context input.txt --model openai/gpt-5-mini\n"
        "\n"
        "Python:\n"
...
def _build_readable_epilogue() -> str:
    parts = [
        "",
        "",
        "def main() -> int:",
...
def _readable_source(
    *,
    ...
    recovered_name = _default_readable_path(replay_path).name
    module_name = recovered_name.removesuffix(".py").replace("-", "_")
    ...
    for block_number, (iteration, code, calls) in enumerate(successful_blocks, 1):
        if block_number > 1:
            code_lines.append("")
        code_lines.append(f"# --- Iteration {iteration} ---")
        for call in calls:
            code_lines.append(_format_llm_call_comment(call_index, call))
            call_index += 1
        if calls:
            code_lines.append("# --- Code using the recorded judgment(s) above ---")
    ...
    epilogue = _build_readable_epilogue()
